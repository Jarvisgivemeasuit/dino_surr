...
import utils_tokens_attn as utils
import vision_transformer_attn_center as vits
from vision_transformer_attn_center import DINOHead
import warnings
...
def get_args_parser():
    ...
    # Misc
    parser.add_argument('--data_path', default='/home/heyj/data/Fewshot_Learning/mini_imagenet/mini_imagenet/train', type=str,
...
def train_dino(args):
    ...
    center_loss1 = CenterLoss(args.local_crops_number + 2, num_classes=100, is_kl=True).cuda()
    # center_loss2 = CenterLoss(2, feat_dim=384,num_classes=351, is_kl=True).cuda()
    ...
    #     dino_loss=dino_loss,
    #     center_loss1=center_loss1,
    #     # center_loss2=center_loss2,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, center_loss1,
        ...
        save_dict = {
            ...
            'center_loss1':center_loss1.state_dict(),
            # 'center_loss2':center_loss2.state_dict(),
            # 'center_head':center_head.state_dict()
        }
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss,center_loss1, data_loader,
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            centerloss1 = center_loss1(student_output, labels) * 16
            # if epoch <= 250:
            #     centerloss2 = center_loss2(student_tokens, labels) * 16 * 0.1
            # if epoch <= 250:
            #     loss = dinoloss + centerloss1 + centerloss2
            # else:
            loss = dinoloss + centerloss1
        ...
        metric_logger.update(clsloss=centerloss1.item())
        # if epoch <= 62:
        #     metric_logger.update(patchloss=centerloss2.item())
...
class CenterLoss(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        for i in range(2):
            ...
            kl = F.kl_div(F.log_softmax(x[i],dim=-1),F.softmax(center,dim=-1),reduction='batchmean')
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number):
        ...
        self.local_transfo = transforms.Compose([
            transforms.RandomResizedCrop(8, scale=local_crops_scale, interpolation=Image.BICUBIC),
            flip_and_color_jitter,
